...
%{
* Matlab looks at everything as matrix. Even a single number/scalar is treated as matrix of 1x1 
* 1d array - Vector 
* 2d array - matrix  
* MATLAB = MATrix LIBrary
* Pi is a built in variable (not a function)
* Typically, 2D are mentioned starting with capital letters. Not mandatory though. Just a convention
* Relationship between these: 
    - Scalars --> Vectors (list of scalars - 1D Array) --> Matrices ( Vector of vectors - 2D Array) --> Arrays (multi dimensional)


* Colon Operator: Gives a vector 
    - <starting point> : <Increments> : <End point> 
    - 1:3:7 % Start with 1, take steps of 3 and end when 7 is reached
    -  If only 2 values are provided then default increment is 1: 1:10 = 1 2 ..1

* Notes:
    - Inputs to an operator are called Operands
    - Inputs to a function are called arguments

    - Colon operator always returns the vector (even when there is no possible value) hence size will be 1x0. However its not same as [ ] as the size of empty matrix is 0x0

* Accessing elements in matrix: 
    - To Access, a(2,3) so 2nd row and 3rd col 
    - Note that indices start with 1 and not 0 like c and java
